...
The `Makefile` you edited above is part of a build system that knows about the dependencies between your source files and .xdc file and the final .bit file.  Every time you change one of them and type `make` it realizes that the "recipe" for your .bit file needs to be re-run because one of the ingredients has changed.  So, what happens if you run `make` and it succeeds in making a .bit file and then you want to run it again to capture the output (to put in a log for example)?   If you type `make` again it will says that everything is up to date and the recipe doesn't need to be re-run.  You can force a re-run by editing one of your source files (put a newline at the bottom or a space somewhere - anything to change the file).  Next time you do `make` it will re-run because one of your files has changed...   
